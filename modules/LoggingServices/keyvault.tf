...
resource "azurerm_private_endpoint" "keyvault_private_endpoint" {
  name                = "${var.name}-${azurerm_key_vault.keyVault.name}-keyvault-private-endpoint"
  location            = var.location
  resource_group_name = var.rgName
  subnet_id           = var.svcSubnetId

  private_service_connection {
    name                           = "${var.name}-${azurerm_key_vault.keyVault.name}-keyvault-private-endpoint-connection"
